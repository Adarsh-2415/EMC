...
} from '@/components/admin/SupportDetailModal'
import { Search, ArrowUpDown, ChevronLeft, ChevronRight, Eye, MessageSquare, Loader2, AlertCircle } from 'lucide-react'

export const AdminSupportPage: React.FC = () => {
  const [records, setRecords] = useState<SupportMessageRecord[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  // Controls
  const [searchQuery, setSearchQuery] = useState('')
  const [sortBy, setSortBy] = useState<'newest' | 'oldest' | 'department'>('newest')
  const [currentPage, setCurrentPage] = useState(1)
  const itemsPerPage = 10

  // Modal Detail State
  const [selectedRecord, setSelectedRecord] = useState<SupportMessageRecord | null>(null)
  ...
  const fetchSupportMessages = useCallback(async () => {
    ...
    try {
      let query = supabase.from('emc_support_messages').select('*')
      ...
      if (err) {
        setError('Failed to fetch support messages. Please check database configuration.')
        setRecords([])
      } else {
        setRecords(data || [])
      }
    } catch {
      setError('An unexpected error occurred while fetching support submissions.')
    } finally {
      setLoading(false)
    }
  }, [sortBy])
    ...
    <div className="space-y-6 max-w-7xl mx-auto select-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200/80 pb-4">
        <div>
          <div className="inline-flex items-center gap-2 text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">
            ...
            <span>Support Enquiries & Tickets</span>
          </div>
        ...
        <span className="text-xs font-bold text-slate-600 bg-white border border-slate-200 px-3.5 py-2 rounded-xl shadow-xs self-start sm:self-auto">
          Total Messages: <span className="text-indigo-600">{records.length}</span>
        </span>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-md border border-slate-200/80 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          <select
            value={sortBy}
            onChange={(e) => setSortBy(e.target.value as 'newest' | 'oldest' | 'department')}
        ...
        {loading ? (
          ...
          <div className="p-12 text-center space-y-3">
            ...
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              {searchQuery
                ? 'No support messages match your search query.'
                : 'There are currently no support messages submitted from the website.'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-200 text-xs text-slate-800 font-medium">
                {paginatedRecords.map((item) => (
                  ...
                  >
                    <td className="py-4 px-6 font-bold text-slate-900">{`${item.first_name} ${item.last_name}`}</td>
                    <td className="py-4 px-6">{item.email}</td>
                    <td className="py-4 px-6">
                      <span className="inline-block px-2.5 py-1 rounded-md bg-indigo-50 text-indigo-700 font-bold border border-indigo-200 text-[11px]">
                        {item.department}
                      </span>
                    </td>
                    <td className="py-4 px-6 max-w-xs truncate">{item.subject}</td>
